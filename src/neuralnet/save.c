/**
 * \file save.c
 * \brief Implementation of save.h
 */
...
void save_nn(struct neural_net *nn, char *path) {
    ...
    fwrite(output->bias_weights, sizeof(double), nn->output_count, file);
    for (int i = 0; i < output->neuron_count; i++) {
        fwrite(output->weights[i], sizeof(double), output->prev_layer_size,
               file);
    }
    fclose(file);
}
...
struct neural_net *load_nn(char *path) {
    ...
    if (!fread(&input_count, sizeof(int), 1, file)) {
        printf("Couldn't read the input count of the neural network from the "
               "file\n");
        exit(1);
    }
    if (!fread(&output_count, sizeof(int), 1, file)) {
        printf("Couldn't read the output count of the neural network from the "
               "file\n");
        exit(1);
    }
    if (!fread(&hidden_layer_count, sizeof(int), 1, file)) {
        printf("Couldn't read the hidden layer count of the neural network "
               "from the file\n");
        exit(1);
    }
    if (!fread(&neuron_count, sizeof(int), 1, file)) {
        printf("Couldn't read the neuron count of the neural network from the "
               "file\n");
        exit(1);
    }

    struct neural_net *nn =
        create_nn(input_count, hidden_layer_count, neuron_count, output_count);

    for (int i = 0; i < hidden_layer_count; i++) {
        if (!fread(nn->layers[i + 1]->bias_weights, sizeof(double),
        ...
        for (int k = 0; k < nn->layers[i + 1]->neuron_count; k++) {
            if (!fread(nn->layers[i + 1]->weights[k], sizeof(double),
                       nn->layers[i + 1]->prev_layer_size, file)) {
    ...
    struct layer *output = nn->layers[nn->hidden_layer_count + 1];
    if (!fread(output->bias_weights, sizeof(double), output->neuron_count,
               file)) {
        printf("Couldn't read the bias weights of the output layer\n");
        exit(1);
    }
    for (int k = 0; k < output->neuron_count; k++) {
        if (!fread(output->weights[k], sizeof(double), output->prev_layer_size,
                   file)) {
