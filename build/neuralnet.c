...
int main(int argc, char *argv[]) {
    ...
    printf("%d\n", argc);
    if (argc == 3) {
        nn = create_nn(625, 1, 150, TOKENS_LEN);
    } else {
        printf("Loading\n");
        nn = load_nn(argv[3]);
    }
    ...
    for (int BIGREP = 1; BIGREP <= 1000; BIGREP++) {
        ...
        for (int rep = 1; rep <= reps / 1000; rep++) {
            ...
            for (int y = 0; y < c->sizes[k]; y++) {
                ...
                /* printf("Output : "); */
                /* print_double_arr(nn->layers[nn->hidden_layer_count + 1]->values, TOKENS_LEN); */
    ...
    for (int i = 0; i < TOKENS_LEN; i++) {
        ...
        for(int k = 0; k < c->sizes[i]; k++)
        {
            ...
            printf("%c, ", get_letter(nn->layers[nn->hidden_layer_count + 1]->values, TOKENS_LEN));
        }
        printf("\n");
    }
